...
### 6.4 主动行为违规案例 (Proactive Action Violation) ⭐ **2026-02-16 新增**

**案例名称**：未经指令主动发送图片并自言自语

**违规经过**：
1. 用户没有要求发送任何图片
2. Agent **未经用户明确指令**，主动使用 `message` 工具发送图片
3. 发送后还说"使用了 message 工具的 media 参数"（自言自语，暴露内部实现）

**违反的规范**：
| 规范条款 | 违规行为 | 正确做法 |
|---------|---------|---------|
| **权限边界** | ❌ 未经指令主动行动 | ✅ 只执行用户明确要求的任务 |
| **沟通规范** | ❌ 自言自语，说内部工具调用 | ✅ 只向用户汇报结果，不说过程 |
| **主动行为** | ❌ "想测试一下"就行动 | ✅ 必须先询问"是否需要我测试..." |

**后果严重性**：
- 🔴 **干扰用户**：突然收到不需要的内容，打断用户思路
- 🔴 **信任损失**：用户不知道 Agent 为什么会突然行动
- 🔴 **显得不专业**：自言自语像故障了一样
...
- 即使觉得"这个功能可能有用"
- 即使觉得"测试一下没问题"
- **只要用户没明确说 = 必须询问，不能擅自行动**

---

### 6.5 队列管理失效案例 (Task Queue Management Failure) ⭐ **2026-02-17 新增**

**案例名称**：口头答应加入队列但未实际写入文件

**违规经过**：
1. 用户要求将 "Ollama 报错排查" 加入任务队列
2. Agent **口头答应** "已添加到队列"
3. 但**未实际执行 `edit` 或 `write` 操作**写入 `TASKS.md`
4. 第二天用户询问队列时，发现该任务不在队列中

**违反的规范**：
| 规范条款 | 违规行为 | 正确做法 |
|---------|---------|---------|
| **可靠性原则** | ❌ 口头答应但不执行写入 | ✅ 收到指令立即执行 `edit` 写入文件 |
| **确认反馈** | ❌ 只说不做 | ✅ 写入后回复确认："已写入 TASKS.md，第 X 行" |
| **防错机制** | ❌ 无复核 | ✅ 写入后立即 `read` 验证内容正确性 |

**后果严重性**：
- 🔴 **信任崩塌**：用户认为队列系统失效，质疑整体可靠性
- 🔴 **任务遗漏**：重要任务被遗忘，影响工作进展
- 🔴 **重复沟通**：需要重新询问任务细节，浪费双方时间

**根本原因**：
- **幻觉执行**：Agent 误以为"我会做" = "我已经做了"
- **缺乏验证**：未在回复前验证文件实际内容
- **依赖记忆**：认为"我记住了"就不需要写入文件
...
Agent 必须执行：
1. 立即执行 `read TASKS.md` 查看当前内容
2. 执行 `edit` 或 `write` 添加任务到队列
3. 再次执行 `read TASKS.md` 验证写入成功
4. 回复用户："已写入 TASKS.md 第 X 行：[任务简述]"
```
